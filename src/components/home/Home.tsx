...
export default function Home() {
  ...
  const handleFilterChange = (newFilters: Partial<Filters>) => {
    setFilters((prev) => {
      ...
      if ("location" in newFilters)
        updated.location = newFilters.location as string;
      if ("minPrice" in newFilters)
        updated.minPrice = Number(newFilters.minPrice) || undefined;
      if ("maxPrice" in newFilters)
        updated.maxPrice = Number(newFilters.maxPrice) || undefined;
  ...
  const filteredPGs = useMemo(() => {
    return pgs.filter((pg) => {
      ...
      if (
        filters.location &&
        !pg.address.toLowerCase().includes(filters.location.toLowerCase())
      ) {
        return false;
      }
    ...
    <div className="min-h-[calc(100vh-73px)]">
      <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-8">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-2">
          ...
          >
            <div className="h-full lg:h-[calc(100vh-120px)] lg:sticky lg:top-[89px]">
